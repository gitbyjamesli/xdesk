...
lazy_static::lazy_static! {
    ...
    pub static ref APP_NAME: RwLock<String> = RwLock::new("xDesk-v3.0".to_owned());
...
impl Config {
    ...
    pub fn get_custom_servers()->Vec<Server>{
        let hh = Self::get_option("server_config_history");
        // log::info!("get_custom_servers1 : {}",hh);
        if hh.is_empty(){
            return vec![]
        }
        let servers: Vec<Server> = match serde_json::from_str(&hh) {
            Ok(data) => data,
            Err(e) => {
        ...
        // let his = serde_json::from_str(&hh);
        // log::info!("get_custom_servers2 : {:?}",his);
    ...
    pub fn get_rendezvous_servers() -> Vec<String> {
        // 优先用自己定义的
        // let ss:Vec<Server> = Self::get_custom_servers();
        // // log::info!("get_custom_servers111 : {:?}",ss);
        // if !ss.is_empty(){
        //     return ss
        //         .into_iter()
        //         .map(|x| x.idServer)
